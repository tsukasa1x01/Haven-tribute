...
function unlockSite(){ unlocked = true; lock.hidden = true; app.hidden = false; initApp(); }
...
function setupAudio(){ document.querySelectorAll('[data-voice]').forEach(button => button.addEventListener('click', () => { document.querySelectorAll('[data-voice]').forEach(item => item.classList.remove('selected')); button.classList.add('selected'); if(activeAudio){activeAudio.pause(); activeAudio = null;} $('#denia-subtitle').textContent = ''; $('#audio-note').textContent = `READY // ${button.dataset.voice.toUpperCase()} AUDIO`; $('#play-voice').disabled = false; })); $('#play-voice').addEventListener('click', () => { const selected = document.querySelector('[data-voice].selected').dataset.voice; if(activeAudio){activeAudio.pause();} activeAudio = new Audio(`assets/audio/denia/birthday-${selected}.mp3`); activeAudio.addEventListener('timeupdate', updateSubtitle); activeAudio.addEventListener('play', () => { $('.celebration-scene').classList.add('is-playing'); $('#audio-note').textContent = `PLAYING // ${selected.toUpperCase()}`; }); activeAudio.addEventListener('pause', () => { $('.celebration-scene').classList.remove('is-playing'); updateSubtitle(); }); activeAudio.addEventListener('ended', finishCelebration); activeAudio.addEventListener('error', () => { $('.celebration-scene').classList.remove('is-playing'); $('#audio-note').textContent = `AUDIO UNAVAILABLE // ADD assets/audio/denia/birthday-${selected}.mp3`; }); activeAudio.play().catch(() => { $('#audio-note').textContent = `AUDIO UNAVAILABLE // ADD assets/audio/denia/birthday-${selected}.mp3`; }); }); }

const RELEASE_AT = Date.parse('2026-09-18T14:02:00Z');
...
if(!oceanCompleted && highestChapter > 5) highestChapter = 5;
...
function unlockSite(){ unlocked = true; lock.hidden = true; app.hidden = false; initApp(); startBackgroundMusic(); showExperienceNotice(); }
function showExperienceNotice(){
  const notice = $('#experience-notice');
  if(!notice || sessionStorage.getItem('shadowArchiveExperienceNotice') === 'seen') return;
  if(window.matchMedia('(max-width: 700px) and (pointer: coarse)').matches) notice.classList.add('phone-entry');
  notice.showModal();
  sessionStorage.setItem('shadowArchiveExperienceNotice', 'seen');
}
...
function renderProgress(){ $('#chapter-progress').innerHTML = chapters.slice(0,7).map((chapter,index)=>{ const inaccessible = !devBypassProgression && (index > highestChapter || (index >= 6 && !oceanCompleted)); return `<button class="progress-dot ${index === 0 ? 'active':''}" data-go="${index}" aria-label="Chapter ${index + 1}: ${chapter}" ${inaccessible ? 'disabled':''}><span>${chapterNumerals[index]}</span></button>`; }).join(''); }
function showChapter(index, remember = true){
  if(index === 7){ if(currentChapter !== 6) showChapter(6, false); return enterEnding(); }
  if(currentChapter === 2 && index !== 2 && activeAudio){ activeAudio.pause(); activeAudio = null; resumeBackgroundMusic(); }
  currentChapter = Math.max(0, Math.min(6,index));
  if(currentChapter !== 5){ stopOceanAutoScroll(); clearOceanAtmosphereTimers(); clearOceanInvestigationTimers(); }
  if(currentChapter === 4) startSubjectAnalysis();
  if(currentChapter === 5) startOceanInvestigation();
  ...
  document.querySelectorAll('.progress-dot').forEach((dot,index) => { dot.classList.toggle('active', index === currentChapter); dot.disabled = !devBypassProgression && (index > highestChapter || (index >= 6 && !oceanCompleted)); });
  if(remember) localStorage.setItem('shadowArchiveChapter', String(currentChapter));
  window.scrollTo({top:0,behavior:'smooth'});
  if(currentChapter === 5 && !oceanCompleted) scheduleOceanEntry();
}
function syncChapterActivity(){ document.querySelectorAll('.chapter video').forEach(video => { if(!video.closest('.chapter')?.classList.contains('active')) video.pause(); }); }
function clearSubjectAnalysisTimers(){ subjectAnalysisTimers.forEach(timer => clearTimeout(timer)); subjectAnalysisTimers = []; }
function startSubjectAnalysis(){
  const investigation = $('#subject-investigation'); const conclusion = $('#subject-conclusion'); const association = $('#association-analysis'); const result = $('#association-result'); const ending = $('#subject-ending');
  if(!investigation || investigation.dataset.started === 'true') return;
  investigation.dataset.started = 'true'; clearSubjectAnalysisTimers();
  const reducedMotion = window.matchMedia('(prefers-reduced-motion: reduce)').matches;
  const steps = reducedMotion ? [80,220,420,700,1000] : [900,1700,2600,3900,5400];
  subjectAnalysisTimers.push(setTimeout(() => { $('#subject-sequence').textContent = 'ANALYSIS INITIALIZED'; }, steps[0]));
  subjectAnalysisTimers.push(setTimeout(() => { document.querySelector('.observation-panel')?.classList.add('is-active'); }, steps[1]));
  subjectAnalysisTimers.push(setTimeout(() => conclusion.classList.add('is-visible'), steps[2]));
  subjectAnalysisTimers.push(setTimeout(() => association.classList.add('is-visible'), steps[3]));
  subjectAnalysisTimers.push(setTimeout(() => { result.hidden = false; association.classList.add('has-result'); }, steps[4]));
  $('#investigate-association').onclick = () => { $('#investigate-association').disabled = true; $('#investigate-association').textContent = 'ACCESSING ASSOCIATED RECORD...'; setTimeout(() => { $('#investigate-association').textContent = 'RECORD UNAVAILABLE. REASON: CLASSIFIED ...FOR NOW.'; ending.hidden = false; ending.classList.add('is-visible'); }, reducedMotion ? 120 : 850); };
}
function clearOceanInvestigationTimers(){ oceanInvestigationTimers.forEach(timer => clearTimeout(timer)); oceanInvestigationTimers = []; }
function startOceanInvestigation(){
  const chapter = $('#chapter-5');
  if(!chapter || chapter.dataset.investigationStarted === 'true') return;
  chapter.dataset.investigationStarted = 'true'; clearOceanInvestigationTimers();
  const reducedMotion = window.matchMedia('(prefers-reduced-motion: reduce)').matches;
  const reveal = selector => $(selector)?.classList.add('is-revealed');
  const steps = reducedMotion ? [40,80,120,160,200] : [900,1800,3000,4300,5600];
  oceanInvestigationTimers.push(setTimeout(() => { $('#ocean-sequence')?.replaceChildren(document.createTextNode('OCEAN ASSOCIATION DETECTED')); chapter.classList.add('association-detected'); }, steps[0]));
  oceanInvestigationTimers.push(setTimeout(() => { $('#ocean-sequence')?.replaceChildren(document.createTextNode('BEGINNING INVESTIGATION...')); reveal('.evidence-primary'); }, steps[1]));
...
function animateOceanProbability(){ const value = $('#ocean-probability-value'); const meter = $('#ocean-probability-meter'); if(!value || !meter) return; const reducedMotion = window.matchMedia('(prefers-reduced-motion: reduce)').matches; const points = [24,48,67,82,91]; if(reducedMotion){ value.textContent = '91%'; meter.style.width = '91%'; return; } points.forEach((point,index) => setTimeout(() => { value.textContent = `${point}%`; meter.style.width = `${point}%`; }, index * 350)); }
...
function startOceanAutoScroll(){
  stopOceanAutoScroll();
  if(window.matchMedia('(prefers-reduced-motion: reduce)').matches || currentChapter !== 5 || oceanCompleted) return;
  ...
  const step = now => {
    if(oceanScrollCancelled || currentChapter !== 5) return;
...
function createParticles(){
  ...
  const count = window.matchMedia('(max-width: 700px) and (pointer: coarse)').matches ? 16 : 38;
  field.innerHTML = Array.from({length:count}, (_, index) => `<i class="particle particle-${index % 6}" style="--x:${(index * 37) % 100}%;--y:${(index * 61) % 100}%;--delay:-${index % 9}s;--duration:${14 + index % 12}s"></i>`).join('');
}
function initSignal(){
...
function renderRecords(){ const stream = $('#record-stream'); if(!stream) return; stream.innerHTML = archiveRecords.map(record => `<article class="record-card"><p class="eyebrow">RECORD // ${record.id}</p><h3>${record.title}</h3><p class="eyebrow">${record.meta}</p><p>${record.text.replaceAll('\n','<br>')}</p><div class="placeholder">STILL TO BE ADDED</div></article>`).join(''); }
...
function showOceanRecovery(){ $('#ocean-incident').classList.add('recovered'); $('#ocean-recovery').hidden = false; oceanCompleted = true; localStorage.setItem('shadowArchiveOceanComplete','true'); highestChapter = Math.max(highestChapter,5); localStorage.setItem('shadowArchiveHighestChapter',String(highestChapter)); renderProgress(); document.querySelectorAll('.progress-dot').forEach((dot,index) => { dot.classList.toggle('active', index === currentChapter); dot.disabled = !devBypassProgression && (index > highestChapter || (index >= 6 && !oceanCompleted)); }); }
...
function renderContributions(){
  ...
  let cardNumber = 0;
  let messageNumber = 0;
  const ordered = [...cards, ...messages];
  $('#contribution-count').textContent = `ARCHIVE CONTRIBUTIONS // ${cards.length + messages.length}`;
  $('#finale-stats').textContent = `${cards.length} RECORDS / ${messages.length} MESSAGES / 1 BIRTHDAY`;
...
function renderRubySurprises(){
  $('#ruby-gallery').innerHTML = rubySurprises.map(item => {
    const images = item.type === 'multi' ? item.images : [item.image];
    return `<article class="ruby-record"><p class="eyebrow">${item.type === 'multi' ? 'MULTI-PAGE RECORD' : 'BIRTHDAY RECORD'}</p><p class="ruby-author">FROM: ${item.from}</p><div class="ruby-images">${images.map((image,index) => `<button class="ruby-image-button" type="button" data-image="assets/images/Ruby/${image}" aria-label="Open birthday wish from ${item.from}, image ${index + 1}"><img src="assets/images/Ruby/${image}" alt="Birthday wish from ${item.from}, image ${index + 1}" loading="lazy"></button>`).join('')}</div></article>`;
  }).join('');
}
function setupFinaleEnhancements(){ document.querySelectorAll('.contribution-art img,.pfp-wrap img,.ruby-image-button img').forEach(image => image.addEventListener('error', () => { image.hidden = true; image.parentElement.classList.add('asset-missing'); })); document.querySelectorAll('.contribution-art,.ruby-image-button').forEach(card => card.addEventListener('click', () => { $('#viewer-image').src = card.dataset.image; $('#viewer').hidden = false; })); $('#viewer').addEventListener('click', event => { if(event.target === event.currentTarget) $('#viewer').hidden = true; }); document.addEventListener('keydown', event => { if(event.key === 'Escape') $('#viewer').hidden = true; }); }
function renderEndingCredits(){ $('#ending-contributor-list').innerHTML = wellWishers.map(person => `<span>${person.name}</span>`).join(''); }
function enterEnding(){ if(endingActive || currentChapter !== 6) return; endingActive = true; const roll = $('#ending-roll'); document.body.classList.add('ending-transition'); setTimeout(() => { app.hidden = true; $('#hidden-ending').hidden = false; window.scrollTo({top:0,behavior:'auto'}); document.body.classList.remove('ending-transition'); roll.classList.remove('is-rolling'); void roll.offsetWidth; roll.classList.add('is-rolling'); }, window.matchMedia('(prefers-reduced-motion: reduce)').matches ? 120 : 1100); }
function replayEnding(){ endingActive = false; $('#hidden-ending').hidden = true; app.hidden = false; showChapter(6,false); window.scrollTo({top:0,behavior:'auto'}); enterEnding(); }
...
function setupDeveloperMode(){
  ...
  const collapse = () => { panel.classList.add('is-collapsed'); dock.hidden = false; panel.setAttribute('aria-expanded', 'false'); };
  const expand = () => { panel.classList.remove('is-collapsed'); dock.hidden = true; panel.setAttribute('aria-expanded', 'true'); };
...
function initApp(){ renderProgress(); renderRecords(); renderContributions(); renderRubySurprises(); syncChapterActivity(); renderEndingCredits(); setupFinaleEnhancements(); createParticles(); initSignal(); renderCompanion(); setupDeveloperMode(); $('#experience-notice-close').addEventListener('click', () => $('#experience-notice').close()); document.addEventListener('click', event => { const target = event.target.closest('[data-next]'); if(target) showChapter(Number(target.dataset.next)); const go = event.target.closest('[data-go]'); if(go && !go.disabled && (devBypassProgression || Number(go.dataset.go) <= highestChapter) && (devBypassProgression || Number(go.dataset.go) < 6 || oceanCompleted)) showChapter(Number(go.dataset.go)); }); $('#enter-archive').addEventListener('click', () => { $('#chapter-0').classList.add('signal-exit'); setTimeout(() => showChapter(1), 320); }); $('#index-records').addEventListener('click', () => showChapter(5)); $('#prevent-merger').addEventListener('click', oceanInteraction); $('#finish-ocean').addEventListener('click', () => showChapter(6)); $('#approach-conclusion').addEventListener('click', enterEnding); $('.wordmark').addEventListener('click', () => showChapter(0,false)); $('#viewer-close').addEventListener('click', () => $('#viewer').hidden = true); $('#sound-toggle').addEventListener('click', event => { backgroundMusicEnabled = !backgroundMusicEnabled; event.currentTarget.setAttribute('aria-pressed', String(backgroundMusicEnabled)); event.currentTarget.textContent = `SOUND / ${backgroundMusicEnabled ? 'ON' : 'OFF'}`; if(backgroundMusicEnabled) resumeBackgroundMusic(); else pauseBackgroundMusic(); }); const savedChapter = Number(localStorage.getItem('shadowArchiveChapter')); const savedHighest = Number(localStorage.getItem('shadowArchiveHighestChapter')); if(savedHighest >= 0) highestChapter = Math.max(highestChapter, Math.min(6,savedHighest)); if(!oceanCompleted && highestChapter > 5) highestChapter = 5; if(savedChapter > highestChapter) highestChapter = Math.min(6,savedChapter); if(savedChapter > 0 && savedChapter < chapters.length && savedChapter <= highestChapter && !devBypassProgression) showChapter(savedChapter,false); updateArchiveTime(); }

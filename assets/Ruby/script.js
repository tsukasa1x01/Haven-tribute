...
Happy birthday again bro. I hope you have a great day and that this next year treats you well. You deserve it. Love ya homie.`,
    date: '2026.09.18'
  }, {
    type: 'card',
    from: 'CONTRIBUTOR',
    image: 'assets/images/contributions/card-placeholder.svg',
    date: '2026.09.18',
    caption: 'Development placeholder card.'
  }, {
    type: 'message',
    from: 'Ruby',
    message: `Hello lil bro! I am glad I met you, we might not be related by blood but I still consider you as my real brother. You are such a kind and caring and amazing person. I want you to live for 150- no 300 years like a tortoise hehe. And don't say you gonna be single forever hmph humph. I want you to find a person who will love you more than anyone in this world.
And last but not least love ya lil bro chuu~
And you will be my eternal slave forever
Happy 20th birthday you little brat!`,
    date: '2026.09.18'
  }, {
    type: 'card',
    from: 'CONTRIBUTOR',
    image: 'assets/images/contributions/card-placeholder-02.svg',
    date: '2026.09.18',
    caption: 'Development placeholder card.'
  }, {
    type: 'message',
    from: 'Nams',
    message: 'Hiii Shadoww! HAPPY BIRTHDAY!!! I hope you spend the day of your birth the best! I wish you more blessings and happiness!!🎂🎉✨\n------ Nams',
    date: '2026.09.18'
  }, {
    type: 'message',
    from: 'Izumi',
...
Once again Happy Birthday and wishing only the best and only the best`,
    date: '2026.09.18'
  }, {
    type: 'message',
    from: 'StormTempest',
    message: `Happy birthday, Shadow, hope your day shines bright,
May laughter find you from morning to night.
Keep chasing your dreams, keep being you,
And may this year bring good things your way too. 🎂`,
    date: '2026.09.18'
  }, {
    type: 'message',
    from: 'Coffee',
...
------ Exclusive coffee supplier aka ZhugeBasha`,
    date: '2026.09.18'
  }, {
    type: 'message',
    from: 'Cheese',
    message: `Many many happy returns of the day, Shadow/void catto nyaa~ 🎂
...
--- by Cheese Catto`,
  ...
  Happy birthday once again, bro! I hope you always stay happy and keep smiling!`,
    ...
    type: 'message',
    from: 'King',
    message: `May this year bring you more happiness
...
-- King/ HosenKing`,
    ...
    type: 'message',
    from: 'Loki',
    message: `Happy birthday, Shadow/void catto nyaa~
Wishing you a full year of unending luck, both in gacha and in life. And thank you so much for taking care of the server because of you, we barely have anything to worry about. Really appreciate you. Happy birthday`,
    date: '2026.09.18'
  }, {
    type: 'message',
    from: 'Minze',
...
May your pity counter be low, your rolls be legendary, and your day be as smooth as your admin logs. Thanks for everything you do to keep this place running — hope today's a certified 5-star day, no pity needed!`,
    date: '2026.09.18'
  }, {
    type: 'message',
    from: 'Kurumii',
    message: `Hello my dear child, :dorodance2:
...
Best regards from this frequent profile changer aka Kurumii :KurumiLove:`,
    date: '2026.09.18'
  }, {
    type: 'message',
    from: 'Kami',
...
const wellWishers = [
  ['Izumi', 'izumi-pfp.png', ['#6da9ff', '#b5d8ff']],
  ['Ruby', 'ruby-pfp.png', ['#ff7897', '#ffd0d8']],
  ['Minze', 'mint-pfp.png', ['#75d7bc', '#d0fff0']],
  ['Shin', 'shin-pfp.gif', ['#8e86ff', '#d2cfff']],
  ['Kuro', 'kuro-catto-nyaa-pfp.gif', ['#7f91ad', '#dbe7f5']],
  ['cheese catto nyaa', 'cheese-catto-nyaa-pfp.png', ['#f1c55e', '#fff0b0']],
  ['Revy', 'revy-pfp.png', ['#d46f73', '#ffc2ae']],
  ['Kurumii', 'kurumii-pfp.png', ['#a77aff', '#e2cfff']],
  ['Nexus', 'nexus-pfp.png', ['#59c6d2', '#c2fbff']],
  ['Kad', 'kad-pfp.png', ['#d28f62', '#ffe0c4']],
  ['Skadi', 'skadi-pfp.png', ['#91b6e9', '#e1efff']],
  ['Saber', 'saber-pfp.png', ['#82d1cb', '#d1fff7']],
  ['Bugs bunny', 'bugs-bunny-pfp.png', ['#c4c9d8', '#f4f5ff']],
  ['Aprillis', 'aprillis-pfp.gif', ['#df9bd1', '#ffe2f7']],
  ['Stormtempest', 'stormtempest-pfp.png', ['#7189d8', '#c4d0ff']],
  ['Gun', 'gun-pfp.png', ['#9c9da9', '#e5e5ef']],
  ['Coffee', 'coffee-pfp.png', ['#b4866a', '#f2d0b5']],
  ['Nams', 'nams-pfp.png', ['#6fb8a5', '#c5f0df']],
  ['King', 'king-pfp.png', ['#d2b26f', '#fff0c9']],
  ['Kami', 'kami-pfp.png', ['#7ab4d9', '#dff5ff']],
  ['Loki', 'loki-pfp.png', ['#7295d8', '#d4e1ff']]
].map(([name, file, palette], index) => ({
  name,
  file,
  palette,
  index
...
function renderLockChapters() {
  $('#lock-chapters').innerHTML = chapters.map((chapter, index) =>
    `<div>${chapterNumerals[index]} ${chapter} <span>🔒</span></div>`).join('');
}
...
function updateArchiveTime() {
  ...
  $('#archive-time').textContent =
    `${pad(now.getUTCDate())}.${pad(now.getUTCMonth() + 1)}.${now.getUTCFullYear()}   ${pad(now.getUTCHours())}:${pad(now.getUTCMinutes())} UTC+7`;
...
function resumeBackgroundMusic() {
  if (backgroundMusicEnabled && !activeAudio) startBackgroundMusic();
}

function renderProgress() {
  $('#chapter-progress').innerHTML = chapters.map((chapter, index) => {
    const inaccessible = !devBypassProgression && (index > highestChapter || (index === 5 && !
      oceanCompleted));
    return `<button class="progress-dot ${index === 0 ? 'active':''}" data-go="${index}" aria-label="Chapter ${index + 1}: ${chapter}" ${inaccessible ? 'disabled':''}><span>${chapterNumerals[index]}</span></button>`;
  }).join('');
}
...
function startOceanAutoScroll() {
  stopOceanAutoScroll();
  if (window.matchMedia('(prefers-reduced-motion: reduce)').matches || currentChapter !== 4 ||
    oceanCompleted) return;
  oceanScrollCancelled = false;
  ['wheel', 'touchstart', 'pointerdown', 'keydown'].forEach(type => document.addEventListener(type,
    stopOceanAutoScroll, {
      passive: true
    }));
  ...
  const targetY = Math.max(startY, target.getBoundingClientRect().top + startY - window
    .innerHeight * .7);
...
function triggerOceanPowerCut(deep) {
  ...
  window.setTimeout(() => chapter.classList.remove('power-cut', 'deep-power-cut'), deep ? 900 :
    520);
}
...
function createParticles() {
  ...
  field.innerHTML = Array.from({
    ...
    `<i class="particle particle-${index % 6}" style="--x:${(index * 37) % 100}%;--y:${(index * 61) % 100}%;--delay:-${index % 9}s;--duration:${14 + index % 12}s"></i>`
  ).join('');
}

function initSignal() {
  const bar = $('#signal-progress-bar');
  const label = $('#signal-label');
  const stages = [$('#signal-confirmation'), $('#signal-name'), $('#signal-occasion'), $(
    '#signal-birthday'), $('#signal-warmth'), $('#signal-subtitle'), $('#enter-archive')];
  const reveal = element => {
    element.classList.add('is-visible');
    element.setAttribute('aria-hidden', 'false');
    if (element === $('#enter-archive')) element.removeAttribute('tabindex');
  };
  const reducedMotion = window.matchMedia('(prefers-reduced-motion: reduce)').matches;
  const timings = reducedMotion ? [100, 180, 260, 340, 460, 580, 700] : [3000, 3140, 4500, 5300,
    6200, 6500, 6900
  ];
  setTimeout(() => {
    bar.style.width = '100%';
  ...
  setTimeout(() => {
    label.textContent = 'IDENTITY FOUND';
  }, reducedMotion ? 40 : 2100);
...
function setupAudio() {
  document.querySelectorAll('[data-voice]').forEach(button => button.addEventListener('click',
    () => {
      ...
      $('#denia-subtitle').textContent = '';
      $('#audio-note').textContent = `READY // ${button.dataset.voice.toUpperCase()} AUDIO`;
      $('#play-voice').disabled = false;
    }));
  $('#play-voice').addEventListener('click', () => {
    const selected = document.querySelector('[data-voice].selected').dataset.voice;
    if (activeAudio) {
      activeAudio.pause();
    }
    pauseBackgroundMusic();
    activeAudio = new Audio(`assets/audio/denia/birthday-${selected}.mp3`);
    activeAudio.addEventListener('timeupdate', updateSubtitle);
    activeAudio.addEventListener('play', () => {
      pauseBackgroundMusic();
      $('.celebration-scene').classList.add('is-playing');
      $('#audio-note').textContent = `PLAYING // ${selected.toUpperCase()}`;
    });
    activeAudio.addEventListener('pause', () => {
      activeAudio = null;
      $('.celebration-scene').classList.remove('is-playing');
      updateSubtitle();
      resumeBackgroundMusic();
    });
    ...
    activeAudio.addEventListener('error', () => {
      ...
      $('#audio-note').textContent =
        `AUDIO UNAVAILABLE // ADD assets/audio/denia/birthday-${selected}.mp3`;
      resumeBackgroundMusic();
    });
...
function triggerSeaFlash() {
  const flash = $('#sea-flash');
  const reducedMotion = window.matchMedia('(prefers-reduced-motion: reduce)').matches;
  const fadeIn = reducedMotion ? 150 : 500;
  const hold = reducedMotion ? 500 : 2000;
  const fadeOut = reducedMotion ? 250 : 3000;
  flash.hidden = false;
  flash.classList.remove('fade-out');
  document.body.classList.add('ocean-contact');
  setTimeout(() => {
  ...
  setTimeout(() => {
    ...
    setTimeout(() => {
      flash.classList.remove('fade-out');
      flash.hidden = true;
      document.body.classList.remove('ocean-contact');
      setTimeout(showOceanRecovery, 650);
    }, fadeOut);
  }, fadeIn + hold);
}

function showOceanRecovery() {
  $('#ocean-incident').classList.add('recovered');
  $('#ocean-recovery').hidden = false;
  oceanCompleted = true;
  localStorage.setItem('shadowArchiveOceanComplete', 'true');
  highestChapter = Math.max(highestChapter, 4);
  localStorage.setItem('shadowArchiveHighestChapter', String(highestChapter));
  renderProgress();
  document.querySelectorAll('.progress-dot').forEach((dot, index) => {
    dot.classList.toggle('active', index === currentChapter);
...
function renderContributions() {
  ...
  let cardNumber = 0;
  let messageNumber = 0;
  $('#contribution-count').textContent = `ARCHIVE CONTRIBUTIONS // ${populated.length}`;
  ...
  $('#wish-stream').innerHTML = messages.map((item, index) =>
    `<article class="wish-note"><p class="eyebrow">WISH // ${String(index + 1).padStart(2,'0')}</p><p>${item.message}</p><p class="author">— ${item.from}</p></article>`
  ).join('');
  $('#well-wisher-board').innerHTML = wellWishers.map(person =>
    `<article class="well-wisher" style="--aura:${person.palette[0]};--aura-soft:${person.palette[1]};--delay:${person.index * -0.35}s"><div class="pfp-wrap"><span class="pfp-particle pfp-particle-a"></span><span class="pfp-particle pfp-particle-b"></span><img src="assets/images/well-wishers/${person.file}" alt="${person.name}" loading="lazy"><span class="pfp-fallback" aria-hidden="true">${person.name.charAt(0)}</span></div><p>${person.name}</p></article>`
  ).join('');
  document.querySelectorAll('.contribution-art').forEach(card => card.addEventListener('click',
    () => {
      $('#viewer-image').src = card.dataset.image;
      $('#viewer').hidden = false;
    }));
}
...
function renderContributions() {
  ...
  $('#well-wisher-board').innerHTML = wellWishers.map(person =>
    `<article class="well-wisher" style="--aura:${person.palette[0]};--aura-soft:${person.palette[1]};--delay:${person.index * -0.35}s"><div class="pfp-wrap"><span class="pfp-particle pfp-particle-a"></span><span class="pfp-particle pfp-particle-b"></span><img src="assets/images/well-wishers/${person.file}" alt="${person.name}" loading="lazy"><span class="pfp-fallback" aria-hidden="true">${person.name.slice(0,1)}</span></div><p>${person.name}</p></article>`
  ).join('');
}

function setupFinaleEnhancements() {
  document.querySelectorAll('.contribution-art img,.pfp-wrap img').forEach(image => image
    .addEventListener('error', () => {
      image.hidden = true;
      image.parentElement.classList.add('asset-missing');
    }));
  $('#viewer').addEventListener('click', event => {
    if (event.target === event.currentTarget) $('#viewer').hidden = true;
  });
...
function showSecurityPass() {
  securityPass.hidden = false;
  lock.hidden = true;
  app.hidden = true;
  securityInput.value = '';
  securityStatus.textContent = 'AWAITING AUTHORITY';
  securityInput.focus();
}
...
function routeAuthority(withTransition = true) {
  ...
  setTimeout(() => {
    securityStatus.textContent = 'ACCESS GRANTED';
    setTimeout(open, 360);
  }, 420);
}
...
function setupDeveloperMode() {
  ...
  const dock = $('#developer-dock');
  const closeButton = $('#developer-panel-close');
  if (authority !== 'KURU') return;
  panel.hidden = false;
  panel.classList.remove('is-collapsed');
  dock.hidden = true;
  panel.setAttribute('aria-expanded', 'true');
  const collapseDeveloperPanel = () => {
    panel.classList.add('is-collapsed');
    dock.hidden = false;
    panel.setAttribute('aria-expanded', 'false');
  };
  const expandDeveloperPanel = () => {
    panel.classList.remove('is-collapsed');
    dock.hidden = true;
    panel.setAttribute('aria-expanded', 'true');
    panel.querySelector('button[data-dev-chapter="0"]')?.focus();
  };
  closeButton.onclick = collapseDeveloperPanel;
  dock.onclick = expandDeveloperPanel;
  devBypassProgression = true;
  renderProgress();
  $('#dev-bypass').addEventListener('click', event => {
    devBypassProgression = !devBypassProgression;
    event.currentTarget.textContent =
      `BYPASS PROGRESSION: ${devBypassProgression ? 'ON' : 'OFF'}`;
    renderProgress();
  });
  $('#dev-reset').addEventListener('click', () => {
    localStorage.removeItem('shadowArchiveChapter');
    localStorage.removeItem('shadowArchiveHighestChapter');
    localStorage.removeItem('shadowArchiveOceanComplete');
    highestChapter = 0;
    oceanCompleted = false;
    showChapter(0, false);
    renderProgress();
  });
  $('#dev-lock').addEventListener('click', () => {
    panel.classList.remove('is-collapsed');
    dock.hidden = true;
    panel.setAttribute('aria-expanded', 'true');
    showSecurityPass();
...
function initApp() {
  ...
  document.addEventListener('click', event => {
    ...
    if (target) showChapter(Number(target.dataset.next));
    const go = event.target.closest('[data-go]');
    if (go && !go.disabled && (devBypassProgression || Number(go.dataset.go) <=
        highestChapter) && (devBypassProgression || Number(go.dataset.go) !== 5 ||
        oceanCompleted)) showChapter(Number(go.dataset.go));
  });
  $('#enter-archive').addEventListener('click', () => {
    $('#chapter-0').classList.add('signal-exit');
    setTimeout(() => showChapter(1), 320);
  });
  $('#index-records').addEventListener('click', () => showChapter(4));
  $('#prevent-merger').addEventListener('click', oceanInteraction);
  $('#finish-ocean').addEventListener('click', () => showChapter(5));
  $('.wordmark').addEventListener('click', () => showChapter(0, false));
  $('#viewer-close').addEventListener('click', () => $('#viewer').hidden = true);
  $('#sound-toggle').addEventListener('click', event => {
    backgroundMusicEnabled = !backgroundMusicEnabled;
    event.currentTarget.setAttribute('aria-pressed', String(backgroundMusicEnabled));
    event.currentTarget.textContent = `SOUND / ${backgroundMusicEnabled ? 'ON' : 'OFF'}`;
    if (backgroundMusicEnabled) resumeBackgroundMusic();
    else pauseBackgroundMusic();
  });
  const savedChapter = Number(localStorage.getItem('shadowArchiveChapter'));
  const savedHighest = Number(localStorage.getItem('shadowArchiveHighestChapter'));
  if (savedHighest >= 0) highestChapter = Math.max(highestChapter, Math.min(5, savedHighest));
  if (!oceanCompleted && highestChapter > 4) highestChapter = 4;
  if (savedChapter > highestChapter) highestChapter = Math.min(5, savedChapter);
  if (savedChapter > 0 && savedChapter < chapters.length && savedChapter <= highestChapter && !
    devBypassProgression) showChapter(savedChapter, false);
  updateArchiveTime();
}
securityInput.addEventListener('input', () => {
  securityInput.value = securityInput.value.toUpperCase();
});
securityForm.addEventListener('submit', authenticate);
$('#early-access-yes').addEventListener('click', () => {
  bakaEarlyAccessGranted = true;
  sessionStorage.setItem('shadowArchiveBakaEarlyAccess', 'true');
  $('#early-access-dialog').close();
  unlockSite();
});
